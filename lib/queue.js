...
function Queue(name, options) {
    ...
    this.collection = mongoose.connection.collection(this.options.collection);
...
Queue.prototype.dequeue = function (options, callback) {
    ...
    this.collection.findAndModify(query, sort, update, {update: true, new: true, upsert: false}, function (err, doc) {
        if (err) return callback(err);
